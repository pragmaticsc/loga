...
We provide one concrete instantiation of the class of languages described above, which we call Loga, to make the experiment tractable. Loga is not itself the hypothesis — it is an existence proof that a language can be deliberately optimised around the computational properties we identify. A different language with the same structural properties would be expected to produce similar results. The full specification is available at https://github.com/pragmaticsc/loga.
