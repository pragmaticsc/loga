...
Large language models are trained almost exclusively on text produced for human readers — text shaped by millennia of communicative evolution, cultural accident, and biological constraint. We argue that this is a contingent, not necessary, choice. The substrate on which a model is trained is a design variable, and natural language is a poor default: it is morphologically irregular, syntactically ambiguous, and tokenizes with poor semantic alignment. We propose a research programme centred on three conjectures. **Conjecture 1**: a small constructed language (*conlang*) designed from the ground up around the computational properties of transformer architectures and byte-pair encoding will yield measurably better training efficiency — lower bits-per-byte at equivalent compute and parameter count — than an English-trained baseline on the same semantic content. **Conjecture 2**: models trained on such a conlang will tolerate aggressive weight quantization (ternary weights ∈ {−1, 0, +1}) better than English-trained models, because the lower information complexity of the training distribution reduces the capacity cost of discretization. **Conjecture 3**: the zeros produced by ternary quantization will cluster at the structural level — entire attention heads, not randomly distributed weights — in conlang-trained models more than in English-trained models, because grammatical regularity enables deeper head specialization; this structured sparsity enables additional post-training compression beyond what ternary quantization alone provides. To make the experiment concrete, we provide one instantiation of this class of languages — a small constructed language we call Loga — and use it as the experimental vehicle. Loga is not itself the hypothesis; it is an existence proof that a language can be deliberately designed around the computational properties we identify. Esperanto and Lojban are other members of the same structural class, but neither was designed with BPE alignment or byte-level efficiency in mind. We ground all three conjectures in recent empirical literature, identify the gap in prior work, and propose a concrete 2×2 factorial experiment using freely available tools and consumer hardware. To our knowledge, the controlled pre-training comparison these conjectures require has not yet been performed. The longer-term motivation is a modular AI architecture in which Loga functions as a structured internal reasoning protocol: a general-purpose LLM translates natural language to Loga at the system boundary; small, specialized reasoning models operate entirely within Loga; a second translation step returns results to natural language. This separates expensive language understanding — performed once per query by a large boundary model — from repeated logical reasoning, which could be executed by domain-specific models of dramatically reduced parameter count if the three conjectures hold.

**Keywords**: language model pre-training, tokenization, constructed language, weight quantization, structured sparsity, bits-per-byte, training efficiency, attention head specialization, modular reasoning, reasoning substrate
...
Galke, Ram, and Raviv (2024) trained recurrent networks and a large language model on artificial languages varying systematically in compositional structure [6]. Languages in which meaning was expressed through regular, transparent combinations of primitives were learned faster, generalized better to held-out combinations, and produced more consistent representations across independently trained agents. This mirrors cognitive science findings on human language acquisition: humans also learn more compositionally regular languages faster and with fewer errors. The neural network behaves, in this respect, as a rational statistical learner — and in the compositional generalization tasks Galke et al. tested, regular substrates improved learning speed and generalization quality.

This finding provides adjacent empirical support for the conlang-for-LLM proposal — it is not direct evidence, since Galke et al. tested compositional generalization on small artificial languages rather than next-token prediction on a translated Wikipedia-scale corpus, and the tasks are categorically different. What it does establish is that neural networks are sensitive to the compositional regularity of their training language in a direction consistent with our hypothesis. The question shifts from pure speculation to a plausible hypothesis with principled motivation: *how much* does regularity help at the scale of modern pre-training and on a realistic corpus?

---

## 3. The Tokenization Problem Is Deeper Than It Appears

Standard practice treats tokenization as a preprocessing detail. BPE [1] is the dominant approach: a greedy algorithm that merges the most frequent byte pairs in the training corpus until a target vocabulary size is reached. BPE is corpus-efficient but linguistically naive. It may split morphologically complex words into multiple subword fragments while merging frequent multi-word phrases into single tokens, based entirely on co-occurrence frequency. Morpheme boundaries are violated routinely and inconsistently.
...
Additional evidence that the reasoning substrate is a genuine design variable comes from Coconut [13], in which researchers at MIT CSAIL and Meta AI Research replaced discrete chain-of-thought tokens with continuous latent vectors. Coconut found that language-space reasoning is suboptimal: the authors report that latent-space reasoning outperforms discrete CoT on logical tasks, which they interpret as evidence that surface textual coherence constraints reduce inference efficiency. Replacing them with unconstrained latent vectors improved performance on logical reasoning tasks. The implication is that natural language imposes costs on reasoning beyond what the reasoning itself requires.
...
We provide one concrete instantiation of the class of languages described above, which we call Loga, to make the experiment tractable. Loga is not itself the hypothesis — it is an existence proof that a language can be deliberately optimised around the computational properties identified in Sections 3–4. A different language with the same structural properties (strict SOV syntax, agglutinative invariant-root morphology, BPE-aligned character partitioning) would be expected to produce similar results. The full grammar is available at https://github.com/pragmaticsc/loga.

**Character set.** Loga uses the 95 printable ASCII characters (U+0020–U+007E), all 1 byte under UTF-8. The space character (U+0020) serves as the word boundary; the remaining 94 characters (`!` through `~`) serve as root characters and grammatical suffixes. Character ranges are partitioned by semantic role: lowercase first character marks noun roots; uppercase marks verb roots; punctuation characters `!`–`/` serve as case suffixes; characters `:`–`@` serve as tense and aspect markers. For lowercase-first tokens the first byte alone identifies noun roots; for uppercase-first tokens, verbs and proper nouns are distinguished by the third byte (tense marker vs. case suffix). This disambiguation is always local and deterministic — the full 3-byte word suffices, with no sentence-level context required.
...
**Morphology.** Loga is agglutinative with invariant roots. An inflected word is exactly 3 bytes: $[\text{root}_1][\text{root}_2][\text{suffix}]$, where the suffix is a case marker for nouns (drawn from the `!`–`/` ASCII range) or a tense marker for verbs (drawn from the `:`–`@` range). No word carries both simultaneously — case and tense are on different word classes. The root never changes form. English's fusional irregularities — go/went, is/was/be, good/better/best — are structurally impossible. The sentence-final period `.` is always a standalone space-delimited token following a complete verb word; the adverbial suffix `.` is always the third byte of a word (noun or verb-root adverb) — these are unambiguous by local structure: verbs cannot take case suffixes, so a `.` following a complete verb word whose third byte is a tense marker is always sentence-final.
...
**Compositionality.** Novel concepts are formed by compounding two roots with a joiner character `{`, preserving compositional transparency: `ku{Ma!` (water-make + nominative) = water-making / irrigation; `ge{se!` (fire-place + nominative) = volcano; `pa{ka!` (idea-person + nominative) = philosopher. Compound class follows the same rule as simple roots — a lowercase first character makes a noun compound, uppercase makes a verb compound — and compounds take the same case or tense suffix as any other word. This follows Galke et al.'s finding [6] that compositional transparency supports generalization, as opposed to logographic expansion which requires each new concept to be memorized.
...
1. **Tokenization alignment**: BPE trained on Loga text will reliably merge 3-byte inflected word forms into single tokens, approaching one token per semantic+grammatical unit. English BPE produces inconsistent multi-token fragments.
...
Ternary weight quantization discretizes each weight to one of three values, reducing weight storage by approximately 10× in theory (the 16-bit to 1.58-bit ratio); published results report 2.6–3.55× whole-model reduction [16] as embeddings and activations remain full precision. This replaces floating-point multiplications with integer additions at inference time [16, 17]. The quantization introduces an accuracy cost proportional to the precision lost. We conjecture that this cost is smaller when the training distribution is more regular: a model encoding a lower-entropy corpus requires less numerical precision to represent the same amount of structure. Put differently, if the weights of an English-trained model are more "loaded" — carrying more bits of information per parameter — then truncating them to ternary loses more.

This conjecture has no direct prior literature, and we acknowledge that the mechanistic argument is not one-directional. A counterargument exists: a model trained on a highly regular language may develop *sharper*, more precisely-peaked weight distributions — because each pattern is learned exactly and without hedging — which could in principle be *more* sensitive to ternary rounding, not less. We conjecture the opposite: that the reduction in total information that must be stored per parameter (a consequence of lower corpus entropy) dominates over any sharpening effect, making each individual weight less critical. But this is genuinely uncertain. The closest indirect evidence is Galke et al. [6], who found that more compositionally regular languages produce more consistent representations across independently trained agents — consistency across seeds suggests the representations occupy a smaller effective volume in weight space, which is consistent with better quantization tolerance. The quantization-tolerance question requires a direct experiment to resolve the direction. A sharper prediction follows from the isoFLOP perspective: cell D (Loga-ternary) should close the gap to cell A (English-float16) more effectively than cell C (English-ternary) does — that is, the val\_bpb penalty of ternary quantization should be smaller as a fraction of the float16 baseline when the training distribution is more regular.
...
All four cells use identical model architecture (small-scale transformer, 10–50M parameters), identical compute budget (automated hyperparameter search via autoresearch-mlx [18] on Apple M4 Max), and BPE tokenizers of identical vocabulary size (8,192) trained on their respective corpora.
...
Ternary training follows the BitNet b1.58 protocol [16]: standard linear layers are replaced with BitLinear layers (absmean quantization to {−1, 0, +1}, straight-through estimator for gradients), with floating-point activations. We use an MLX-native `BitLinear` implementation (included in the repository at `train/bitlinear.py`) rather than the official CPU `bitnet` reference library; MLX's Metal kernels enable efficient quantization-aware training on Apple Silicon without requiring CUDA. Fully ternary activations are not used; activation quantization introduces additional accuracy penalties orthogonal to the hypotheses under test, and the BitNet b1.58 protocol retains full-precision activations for this reason [16]. A technical note: the HuggingFace `ByteLevel` pre-tokenizer used in both conditions maps each input byte to a Unicode surrogate before BPE training (following the GPT-2 byte-to-Unicode mapping). Loga's ASCII character ranges are thus remapped to Unicode code points before merging; the statistical clustering properties that distinguish noun and verb tokens are preserved under this bijective transform, and the mapping applies identically to both conditions.
...
The mechanism operates at the level of attention head specialization. Prior work has established that transformer attention heads divide labour, with individual heads specializing in specific syntactic functions — positional dependencies, rare tokens, syntactic agreement — and that heads which do not specialize can be pruned with minimal accuracy cost [19, 20]. Crucially, this specialization is more complete when the linguistic patterns to be learned are more regular.
...
1. **Higher head-level zero variance in Loga models.** Because Loga's grammar is context-free and locally decodable from byte classes, the number of distinct syntactic circuits required is bounded by the finite state machine implicit in the EBNF. We expect some heads to become highly sparse (those whose narrow syntactic pattern is fully handled by a small set of consistent rules) while others remain dense (those handling lexical composition and long-range structure). This predicts high variance across heads — and potentially a bimodal distribution, though we make no strong commitment to a specific distributional shape. English-trained models should show lower variance, as residual ambiguity forces every head to retain some diffuse capacity rather than specialising completely.
...
- Apply magnitude-based head pruning at several incremental thresholds. Record val\_bpb after each pruning step for both models.
- Compare the pruning degradation curves: a shallower curve for cell D than cell C supports Conjecture 3.

**Learning curve measurement.** All four cells log val\_bpb at regular training step intervals, not only at convergence. Plotting val\_bpb against training steps (compute) directly tests sample efficiency: if the Loga cells (B and D) reach a given val\_bpb threshold in fewer steps than their English counterparts (A and C), this supports the interpretation that a more regular training substrate reduces the data required to learn equivalent structure — the core prediction underlying all three conjectures. We further predict that the advantage will be visible early: Loga's vocabulary structure ensures that noun tokens and verb tokens occupy disjoint regions of the BPE vocabulary — the model receives an approximately deterministic prior on syntactic category from the token identity alone, which should accelerate the emergence of coherent attention patterns within the first epoch on Wikipedia-scale data.

---

## 8. Implications and Limitations

If Conjecture 1 is supported, it implies that the choice of training language is an unexploited lever on model efficiency. Pre-training corpora could be designed or curated for tokenization alignment, not just scale and quality. If Conjecture 2 is supported, it implies a design principle for quantized models: train on simpler, more regular data to make the quantization penalty cheaper. If Conjecture 3 is supported, the implications compound: the three techniques — conlang training, ternary quantization, and structured pruning — form a compression cascade in which each step makes the next cheaper, potentially enabling capable models at parameter counts previously considered inadequate.

A concrete application motivating this cascade is a modular reasoning architecture. Loga is not intended as an output language — it is an internal protocol, analogous to a compiler's intermediate representation. A general-purpose LLM (already available) translates natural language inputs into Loga at the system boundary; one or more small Loga-native models perform domain-specific reasoning entirely within Loga, passing structured, unambiguous reasoning chains between them; a second translation step decodes the result to natural language. This design separates language understanding — expensive, general-purpose, executed once per query — from logical reasoning — cheap, specializable, executed many times. If the compression cascade holds, each reasoning expert in such a network could operate at dramatically reduced parameter counts, deployable on consumer hardware or edge devices, while the network as a whole handles queries that would otherwise require a single large model. The Coconut work [13] demonstrated that reasoning in a non-linguistic space outperforms reasoning in English tokens; Loga offers a structured, discrete, and transmissible alternative to continuous latent vectors — one that can be passed between independently trained models, routed deterministically from its first byte, and inspected by researchers.

A question worth addressing before turning to experimental limitations: why a purpose-built conlang rather than an existing formal language — Prolog, Python, or a typed lambda calculus — which already possess context-free grammar, unambiguous syntax, and compositional semantics? Formal languages differ in two important ways. First, their token distributions are sparse: programs use a small fraction of available syntax, and vocabulary is dominated by user-defined identifiers with no distributional regularity across documents. A model trained on Python learns mostly variable names, not compositional semantic structure. Second, translation to Loga is feasible because Loga is designed to be NL-adjacent, relying on the same reformulation capabilities LLMs already possess; reliable translation to arbitrary sparse formal languages is difficult because such languages are underrepresented in LLM pre-training data. Loga is designed to be simultaneously LLM-translatable and computationally regular — a combination existing formal languages do not satisfy.
...
A third limitation is sequence length. Loga's explicit morphology — encoding "went" as root + tense suffix rather than a single token — may expand token sequences for equivalent semantic content. Longer sequences raise attention cost (O(n²)) and, within autoresearch-mlx's fixed 5-minute budget, reduce the number of optimizer steps per run. Two factors mitigate this. First, the primary metric val_bpb normalises by bytes of *input*, not number of tokens, so a longer sequence that compresses better still registers a lower val_bpb; the metric directly captures whether the added length is earning its keep. Second, BPE trained on Loga text will merge frequent 3-byte inflected word forms into single tokens, recovering much of the sequence length that morphological explicitness adds. The experiment will report tokens-per-article alongside val_bpb to make the sequence length effect directly observable.

A further limitation is effective vocabulary size. Loga's core vocabulary of ~300 noun roots and ~200 verb roots is dramatically smaller than English's open vocabulary. A model predicting over a smaller effective vocabulary has lower next-token entropy by construction, independent of any architectural advantage from grammatical regularity. This confound may be the dominant effect: predicting over ~500 roots differs categorically from predicting over English's tens of thousands of distinct word forms. Byte-normalised bpb accounts for differences in encoding length but not for differences in intrinsic prediction difficulty arising from vocabulary size. A positive result on Conjecture 1 should therefore be interpreted with caution — it is consistent with both "more efficient substrate" and "intrinsically easier prediction task." Reporting the type-token ratio and effective vocabulary size of each corpus alongside val_bpb will help characterise this confound; fully disentangling the effects would require a control condition with a vocabulary-matched English baseline, such as a Basic English 850-word constraint, which is left for follow-on work.
...
A further consideration is scale. The 3B-parameter scale tested in Ma et al. [16] is where BitNet b1.58 performance matches float16 baselines; this is a single data point rather than a characterised threshold, and the 2B4T report [17] later extended results to 2B parameters. The gap between ternary and float16 narrows as scale increases [16]. At 10–50M parameters, Conjecture 2's interaction effect may be difficult to observe against the baseline quantization penalty. We consider this an acceptable limitation for a first experiment: the within-ternary comparison (cell D versus cell C) is well-defined regardless of whether either ternary model matches its float16 counterpart in absolute terms.

A broader scope limitation: Loga is one point in a large design space. The experiment tests whether languages of this structural type outperform English; a positive result would support the design principles Loga instantiates, not Loga specifically. Follow-on work could test other instantiations — including Esperanto or Lojban — to characterise which design properties matter most and to what degree.
...
A scope limitation: val_bpb is a compression proxy. For a Loga expert operating within a modular pipeline, downstream transfer to natural language is not required — the boundary LLM handles that. For a Loga model used standalone, whether low bpb implies useful reasoning capability is a separate empirical question, tested by fine-tuning on translated benchmarks and left for subsequent work.

Finally, the modular architecture introduces a translation cost not captured by val_bpb. The boundary LLM is itself a large model, and whether the system-level gain from cheap reasoning steps outweighs the fixed per-query translation cost depends on how many reasoning steps each query requires. For tasks involving more than a handful of inference steps, the economics strongly favour small reasoning models; quantifying the break-even is left for systems-level analysis once the core conjectures are established.
...
A converging body of evidence — from compositional learning theory [6], tokenization information theory [7, 8], multilingual representation probing [9], continuous reasoning research [13], and attention head specialization studies [19, 20] — now provides principled grounds for all three conjectures. What remains is the experiment. The required tools — autoregressive pre-training at modest scale, automated hyperparameter search, BPE tokenizer training, LLM-assisted corpus translation — are all freely available, and the compute required is accessible on consumer hardware.

The three conjectures form a convergent structure rooted in Conjecture 1. If training language regularity is a design variable that reduces representational burden (Conjecture 1), two independent consequences follow: the reduced information per parameter makes quantization cheaper (Conjecture 2), and the more complete head specialisation that regularity enables produces more structured sparsity after quantization (Conjecture 3). Conjectures 2 and 3 are therefore parallel predictions from the same root cause, not a strict causal chain in which each step depends on the previous. Each is individually testable and individually useful; together, if supported, they describe a compression cascade — conlang training, ternary quantization, and structured pruning — in which each technique extracts further efficiency from the regularity that the training language makes available.

The experiment proposed here is the first step toward validating this architecture. Before building the modular pipeline, we need to know whether Loga-trained models are in fact more efficient per parameter (Conjecture 1), whether they tolerate the quantization that makes small experts practical (Conjecture 2), and whether they exhibit the structured sparsity that makes pruning cheap (Conjecture 3). A positive result on all three does not prove the architecture works — that requires a separate systems experiment — but it establishes the foundation. A null result is equally informative: it would indicate that grammatical regularity alone is insufficient, and that the efficiency gains must be sought elsewhere.
...
[19] Voita, E. et al. (2019). Analyzing Multi-Head Self-Attention: Specialized Heads Do the Heavy Lifting, the Rest Can Be Pruned. *ACL 2019*. https://aclanthology.org/P19-1580/

[20] Michel, P., Levy, O. & Neubig, G. (2019). Are Sixteen Heads Really Better than One? *NeurIPS 2019*. arXiv:1905.10650.
