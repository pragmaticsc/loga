...
A question worth addressing directly is why a purpose-built conlang is preferred over an existing formal language — Prolog, Python, or a typed lambda calculus — which already possess many of Loga's structural properties: context-free grammar, unambiguous syntax, and compositional semantics. Formal languages differ in two important ways. First, their token distributions are sparse: programs use a small fraction of available syntax, and vocabulary is dominated by user-defined identifiers carrying no distributional regularity across documents. A language model trained on Python source code learns mostly variable names, not compositional semantic structure. Second, reliable natural-language-to-formal-language translation remains an unsolved problem at scale; the boundary translation step the modular architecture requires is only feasible because LLMs already handle natural-language-to-natural-language reformulation. Loga is designed to be simultaneously LLM-translatable and computationally regular — a combination that existing formal languages do not satisfy.

A further consideration is downstream task generalization. val_bpb on held-out Loga text is a compression proxy, not a direct measure of reasoning capability. A Loga-trained model that achieves low bpb may have learned Loga-specific structural patterns without developing transferable representations. Whether Loga pre-training supports downstream reasoning is a distinct empirical question, tested by fine-tuning on translated reasoning benchmarks; that experiment is left for subsequent work, and the present paper makes no claim that low bpb implies reasoning capability.

Finally, the modular architecture introduces a translation cost that the efficiency analysis does not fully account for. The boundary LLM that translates natural language to Loga and back is itself a large, expensive model. Whether the system-level efficiency gain — small Loga reasoning models executing many inference steps cheaply — outweighs the fixed translation cost per query depends on the ratio of reasoning steps to boundary calls, a quantity that varies by task. This system-level trade-off is left for empirical analysis once the core conjectures are established.
